...
st.title("🧬 Lab Report 2: Genetic Algorithm")

# Display student info
st.write(f"**Name:** {STUDENT_NAME}")
st.write(f"**ID Number:** {STUDENT_ID}")
...
with st.expander("ℹ️ Problem setup (fixed)", expanded=True):
    st.write(
        f"""
...
if run_btn:
    ...
    # Using a simple line chart for the convergence curve since matplotlib was removed
    st.subheader("📈 Convergence Curve (Best Fitness per Generation)")
    st.line_chart(curve)  # Using st.line_chart for plotting without matplotlib
